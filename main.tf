...
# 2️⃣ Create Subnet (Depends on VPC)
...
# 3️⃣ Create IAM Service Account (Depends on VPC)
...
# 4️⃣ Attach IAM Role to Service Account (Depends on SA)
...
# 5️⃣ Compute Engine Instance (Depends on IAM and Network)
